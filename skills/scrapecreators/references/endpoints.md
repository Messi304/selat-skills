...
Multi-merchant social scraping across **SELAT-native** (X/Twitter — catalog.selat.ai, routed via SELAT Router, Circle Gateway-batched), **StableSocial** (Instagram + TikTok — MPP, routed via the SELAT Router), and **Clado** (LinkedIn — MPP, routed). Every endpoint below is probe-verified live-payable (`selat-pay --probe-only`, 2026-07-10). Caps (`maxAmount`) are ~10x each live price, not the live price.

## Endpoints used

| # | Step | Method | URL | Rail | Live price |
|---|---|---|---|---|---|
| 1 | X/Twitter profile — SELAT-native | GET | `https://catalog.selat.ai/twitter/user/info?userName=${handle}` | routed | $0.001 |
| 2 | X/Twitter user tweets — SELAT-native | GET | `https://catalog.selat.ai/twitter/user/last_tweets?userName=${handle}` | routed | $0.001 |
| 3 | X/Twitter tweet details — SELAT-native | GET | `https://catalog.selat.ai/twitter/tweets?tweet_ids=${tweetId}` | routed | $0.001 |
